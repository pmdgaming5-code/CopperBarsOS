...
Manages per-application Wine prefixes and launches Windows PE executables
without exposing the host filesystem through a shared prefix by default.
"""
import json
import os
import pathlib
...
class WindowsCenter(tk.Tk):
    def __init__(self):
        ...
        tk.Label(self, text="EXE dosyaları için Wine tabanlı güvenli uyumluluk merkezi",
        ...
        self.button(actions, "Prefix'leri Yenile", self.refresh).pack(side=tk.RIGHT)
    ...
    def new_prefix(self):
        name = filedialog.asksaveasfilename(title="Prefix adı", initialfile="WindowsApp", defaultextension="")
        if not name:
            return
    ...
    def choose_exe(self):
        path = filedialog.askopenfilename(title="Windows uygulaması seç", filetypes=[("Windows EXE", "*.exe"), ("Tüm dosyalar", "*")])
        if not path:
            return
    ...
    def launch_exe(self, path):
        try:
            ...
            prefix = prefix_for(app_name)
            env = init_prefix(prefix)
            ...
            self.after(0, lambda: self.status.configure(text="Uygulama hâlâ çalışıyor; başlatma aracı zaman aşımına uğramadı fakat izleme durduruldu."))
        except Exception as exc:
