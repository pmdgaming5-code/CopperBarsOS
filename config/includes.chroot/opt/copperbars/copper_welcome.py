...
def ollama_available():
    return subprocess.run(
        ["sh", "-c", "command -v ollama >/dev/null 2>&1"],
        capture_output=True,
    ).returncode == 0
...
def pull_model(name):
    if not ollama_available():
        raise RuntimeError(
            ...
            "Copper Store üzerinden yerel AI altyapısını kurabilirsin."
        )
...
class Welcome(tk.Tk):
    def __init__(self):
        ...
        tk.Label(
            self,
            text="Bilgisayarını birkaç adımda hazırla",
            bg=BG,
            fg=TEXT,
        ...
        tk.Label(
            self,
            text=f"Algılanan RAM: {ram_gb():.1f} GB • Mimari: {platform.machine()}",
            bg=BG,
            fg=MUTED,
        ).pack(pady=(5, 20))
        ...
        tk.Label(
            card,
            text="İlk Copper AI modelini seç",
            bg=PANEL,
            fg=TEXT,
        ...
        tk.Label(
            card,
            text="Model dosyaları Git'e gömülmez; seçtiğin model yerel olarak kullanılmak üzere ayarlanır.",
            bg=PANEL,
            fg=MUTED,
            wraplength=780,
            justify="left",
        ...
        for model_id, title, size, desc in MODELS:
            ...
            tk.Radiobutton(
                ...
                fg=TEXT,
                selectcolor="#11161d",
                activebackground="#11161d",
                activeforeground=TEXT,
                font=("DejaVu Sans", 11, "bold"),
                text=f"{title}  •  {size}",
            ).pack(anchor="w", padx=14, pady=(10, 0))
            tk.Label(row, text=desc, bg="#11161d", fg=MUTED).pack(
                anchor="w", padx=43, pady=(0, 10)
            )
        ...
        tk.Button(
            buttons,
            text="Seç ve Modeli İndir",
            command=self.save_and_pull,
            bg="#555d68",
            fg=TEXT,
            relief=tk.FLAT,
            padx=18,
    ...
    def save_only(self):
        try:
            ...
            messagebox.showinfo(
                "CopperBarsOS",
                f"{self.selected.get()} seçildi. Copper yeniden başlatıldığında bu modeli kullanacak.",
            )
            self.destroy()
