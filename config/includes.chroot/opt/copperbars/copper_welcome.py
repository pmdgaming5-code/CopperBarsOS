#!/usr/bin/env python3
"""CopperBars first-run setup and Copperium AI model selector."""
import platform
import subprocess
...
def save_model(name):
    script = 'printf "%s\\n" "$1" > /var/lib/copperbars/model.conf'
    command = ["pkexec", "/bin/sh", "-c", script, "copper-model", name]
...
def pull_model(name):
    if not ollama_available():
        raise RuntimeError(
            "Ollama bu kurulumda bulunamadı. Model seçimi yine kaydedildi; "
            "CopperBars Store üzerinden yerel AI altyapısını kurabilirsin."
        )
...
class CopperBarsSetup(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("CopperBars Setup — CopperBarsOS")
        self.geometry("900x650")
        self.minsize(720, 560)
        self.configure(bg=BG)
        self.selected = tk.StringVar(value="qwen3:1.7b")

        tk.Label(self, text="CopperBars Setup", bg=BG, fg=ACCENT,
                 font=("DejaVu Sans", 28, "bold")).pack(pady=(28, 4))
        tk.Label(self, text="Bilgisayarını ve Copperium AI'ı birkaç adımda hazırla", bg=BG, fg=TEXT,
                 font=("DejaVu Sans", 15, "bold")).pack()
        tk.Label(self, text=f"Algılanan RAM: {ram_gb():.1f} GB • Mimari: {platform.machine()}",
                 bg=BG, fg=MUTED).pack(pady=(5, 20))

        card = tk.Frame(self, bg=PANEL)
        card.pack(fill=tk.BOTH, expand=True, padx=34, pady=10)
        tk.Label(card, text="Copperium AI için model seç", bg=PANEL, fg=TEXT,
                 font=("DejaVu Sans", 16, "bold")).pack(anchor="w", padx=22, pady=(20, 5))
        tk.Label(card, text="Model ağırlıkları Git'e gömülmez. Seçim yerel kullanım için yapılandırılır.",
    ...
    def save_only(self):
        try:
            ...
            messagebox.showinfo("CopperBars Setup", f"{self.selected.get()} seçildi. Copperium AI bu modeli kullanacak.")
            self.destroy()
        except Exception as exc:
            messagebox.showerror("CopperBars Setup", str(exc))

    def save_and_pull(self):
        try:
            save_model(self.selected.get())
            pull_model(self.selected.get())
            messagebox.showinfo("CopperBars Setup", "Model kuruldu ve Copperium AI için seçildi.")
            self.destroy()
        except Exception as exc:
            messagebox.showerror("CopperBars Setup", str(exc))
